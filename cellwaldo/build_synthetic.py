...
# Uses cellwaldo.png (make_cell_waldo.py) and histology_backgrounds/* to generate synthetic dataset
...
N_SAMPLES   = 8000               # Adjust as desires
IMG_SIZE    = 640                # Square canvas (YOLO)
...
# coarse augmentations for Waldo *before* paste
waldo_aug = A.Compose([
...
for i in tqdm(range(N_SAMPLES)):
    ...
    # central crop / resize   (keeps aspect & cell detail)
    h, w = bg.shape[:2]
    c = min(h, w)
    ...
    # save YOLO label  (class 0)
    # YOLO expects normalized centre-x, centre-y, width, height
    cx = (x + ww/2) / IMG_SIZE
    cy = (y + wh/2) / IMG_SIZE
    bw = ww / IMG_SIZE
    bh = wh / IMG_SIZE
